...
- **Backend Swappability**: Future optimization work must maintain deterministic execution. When adding a compiled backend or updating the execution path, the existing validation (tests, API contracts) must remain intact. Any backend must support testing against the reference implementation to ensure phase and magnitude equivalency. The Python `execute_python_fir_2d` implementation remains the explicit specification. The `PARTITIONED` overlap-save path serves as the explicit blueprint for future compiled C++ migrations. Native compilation is strictly optional.
- **FIR Memory Contract**: Any future C++ extensions must respect the explicit `float64`, channel-first `[channels, samples]`, C-contiguous boundary established in `fir_executor.py`. `mode="same"` alignment behavior is non-negotiable and must match the Python reference exactly.
